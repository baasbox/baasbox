...
public abstract class AbstractAdminAssetTest extends AbstractTest
{
		...
		(
			...
			new Runnable() 
			{
				public void run() 
				{
					...
					request = request.withHeader(TestConfig.KEY_APPCODE, TestConfig.VALUE_APPCODE);
					request = request.withHeader(TestConfig.KEY_AUTH, "Basic YWRtaW46YWRtaW");
					result = routeAndCall(request);
		...
		(
			...
			new Callback<TestBrowser>() 
	        {
				public void invoke(TestBrowser browser) 
				{
					...
					// Invalid Authorization
					setHeader(TestConfig.KEY_APPCODE, TestConfig.VALUE_APPCODE);
					setHeader(TestConfig.KEY_AUTH, "Basic YWRtaW46YWRtaW");
					httpRequest(getURLAddress(), getMethod());
